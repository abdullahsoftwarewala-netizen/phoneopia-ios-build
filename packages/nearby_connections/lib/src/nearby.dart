...
class Nearby {
  ...
  Future<int> sendBytesPayload(String endpointId, Uint8List bytes) async {
    final payloadId = await _channel.invokeMethod<dynamic>(
      'sendPayload',
      <String, dynamic>{
        'endpointId': endpointId,
        'bytes': bytes,
    ...
    return (payloadId as num).toInt();
  }

  /// Write one framed call-audio chunk into a long-lived Nearby STREAM.
  /// Native code creates the stream lazily on the first frame and keeps only
  /// current audio when the radio is temporarily slower than capture.
  Future<int> sendAudioStreamChunk(String endpointId, Uint8List bytes) async {
    final payloadId = await _channel.invokeMethod<dynamic>(
  ...
  Future<void> stopAudioStream(String endpointId) => _channel.invokeMethod(
        'stopAudioStream',
